 ...
 *
 * @param {SourceMap} nextSourceMap Source map for a new transformation applied after the current source map.
...
SourceMap.prototype.apply = function(nextSourceMap, nextSourceFilename) {
    var currentMap = new SourceMapConsumer(this.toString());
    var nextMap    = new SourceMapConsumer(nextSourceMap.toString());
...
/* NOT NEEDED ANYMORE
    var generator = new SourceMapGenerator({
        file: nextSourceFilename
    });

    // Copy sourcesContent too
    (this.sourcesContent || []).forEach(function(contents, i) {
        var sourcePath = this.sources[i];
        generator.setSourceContent(sourcePath, contents);
    }.bind(this));

    // Rebase the next mapping from the originalMap (if any)
    nextMap.eachMapping(function(minimiseMapping) {
        var original = currentMap.originalPositionFor({
